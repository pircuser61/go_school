...
func (ae *APIEnv) ModuleUsage(w http.ResponseWriter, req *http.Request, moduleName ModuleName) {
	...
	err = sendResponse(w, http.StatusOK, entity.UsageResponse{Name: string(moduleName), Pipelines: usedBy, Used: used})
	if err != nil {
		e := UnknownError
		log.Error(e.errorMessage(err))
		_ = e.sendError(w)
...
func (ae *APIEnv) ModuleRun(w http.ResponseWriter, req *http.Request, moduleName ModuleName) {
	...
	for i := range eriusFunctions {
		if eriusFunctions[i].Title == string(moduleName) {
