...
func (ae *APIEnv) CreatePipelineVersion(w http.ResponseWriter, req *http.Request, pipelineID PipelineID) {
	...
	p.ID, err = uuid.Parse(string(pipelineID))
	if err != nil {
...
func (ae *APIEnv) RunVersion(w http.ResponseWriter, req *http.Request, versionID VersionID) {
	ctx, s := trace.StartSpan(req.Context(), "run_pipeline")
	defer s.End()

	log := logger.GetLogger(ctx)

	id, err := uuid.Parse(string(versionID))
	if err != nil {
		e := UUIDParsingError
		log.Error(e.errorMessage(err))
		_ = e.sendError(w)
...
func (ae *APIEnv) RunVersionsByBlueprintID(w http.ResponseWriter, r *http.Request) {
	...
	ctx = context.WithValue(ctx, pipeline.SdApplicationDataCtx{}, pipeline.SdApplicationData{
		...
		ApplicationBody: map[string]interface{}(req.ApplicationBody),
	})
...
func (ae *APIEnv) DeleteVersion(w http.ResponseWriter, req *http.Request, versionID VersionID) {
	ctx, s := trace.StartSpan(req.Context(), "delete_version")
	defer s.End()

	log := logger.GetLogger(ctx)

	vID, err := uuid.Parse(string(versionID))
	if err != nil {
		e := UUIDParsingError
		log.Error(e.errorMessage(err))
		_ = e.sendError(w)
...
func (ae *APIEnv) GetPipelineVersion(w http.ResponseWriter, req *http.Request, versionID VersionID) {
	ctx, s := trace.StartSpan(req.Context(), "get_version")
	defer s.End()

	log := logger.GetLogger(ctx)

	versionUUID, err := uuid.Parse(string(versionID))
	if err != nil {
		e := UUIDParsingError
		log.Error(e.errorMessage(err))
		_ = e.sendError(w)
