...
func (ae *APIEnv) DebugTask(w http.ResponseWriter, req *http.Request, workNumber WorkNumber) {
	...
	task, err := ae.DB.GetTask(ctx, string(workNumber))
	if err != nil {
		e := GetTaskError
		log.Error(e.errorMessage(err))
		_ = e.sendError(w)
...
func (ae *APIEnv) LastVersionDebugTask(w http.ResponseWriter, req *http.Request, versionID VersionID) {
