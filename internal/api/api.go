...
const (
	POST EndSystemSettingsMethod = "POST"
)
...
const (
	N125 ProcessSettingsWorkType = "12/5"
	N247 ProcessSettingsWorkType = "24/7"
	N85  ProcessSettingsWorkType = "8/5"
)
...
const (
	CompareBooleanOperatorEqual    CompareBooleanOperator = "Равно"
	CompareBooleanOperatorNotEqual CompareBooleanOperator = "Не равно"
)

// Defines values for CompareDateOperator.
const (
	CompareDateOperatorEqual       CompareDateOperator = "Равно"
	CompareDateOperatorLess        CompareDateOperator = "Меньше"
	CompareDateOperatorLessOrEqual CompareDateOperator = "Меньше или равно"
	CompareDateOperatorMore        CompareDateOperator = "Больше"
	CompareDateOperatorMoreOrEqual CompareDateOperator = "Больше или равно"
	CompareDateOperatorNotEqual    CompareDateOperator = "Не равно"
)

// Defines values for CompareIntegerOperator.
const (
	CompareIntegerOperatorEqual       CompareIntegerOperator = "Равно"
	CompareIntegerOperatorLess        CompareIntegerOperator = "Меньше"
	CompareIntegerOperatorLessOrEqual CompareIntegerOperator = "Меньше или равно"
	CompareIntegerOperatorMore        CompareIntegerOperator = "Больше"
	CompareIntegerOperatorMoreOrEqual CompareIntegerOperator = "Больше или равно"
	CompareIntegerOperatorNotEqual    CompareIntegerOperator = "Не равно"
)

// Defines values for CompareNumberOperator.
const (
	CompareNumberOperatorEqual       CompareNumberOperator = "Равно"
	CompareNumberOperatorLess        CompareNumberOperator = "Меньше"
	CompareNumberOperatorLessOrEqual CompareNumberOperator = "Меньше или равно"
	CompareNumberOperatorMore        CompareNumberOperator = "Больше"
	CompareNumberOperatorMoreOrEqual CompareNumberOperator = "Больше или равно"
	CompareNumberOperatorNotEqual    CompareNumberOperator = "Не равно"
)

// Defines values for CompareStringOperator.
const (
	Contain    CompareStringOperator = "Содержит"
	Equal      CompareStringOperator = "Равно"
	NotContain CompareStringOperator = "Не Содержит"
	NotEqual   CompareStringOperator = "Не равно"
)
...
// Defines values for GetTasksParamsExecutorTypeAssigned.
const (
	AssignedByMe GetTasksParamsExecutorTypeAssigned = "assigned_by_me"
	AssignedToMe GetTasksParamsExecutorTypeAssigned = "assigned_to_me"
)
...
// ArrayItems Описание типа, который хранится в массиве. Если type = object, тогда поле properties обязательное. Оно нужно для описание конкретного типа объектов, которые хранятся в массиве. Если type = array(многомерный массив у нас то есть), тогда поле items обязательное. Оно описывает тип массивов.
type ArrayItems struct {
	...
	// Properties Представляет из себя набор ключ-значение, где ключ - это название переменной/поля объекта, а значение - это структура, которая описывает переменную(или поле объекта). Причём, если переменная - это объект, тогда должно быть заполнено поле propeties(описание полей). Если переменная - массив, тогда должно быть заполнено поле items(описание типа, который хранится в массиве).
...
type EndSystemSettings struct {
	...
	MicroserviceId openapi_types.UUID      `json:"microservice_id"`
}
...
type ExecutableFunctionParams struct {
	// Constants Constant values for function parameters (key of object is variable name)
	Constants map[string]interface{} `json:"constants"`

	// Mapping Представляет из себя набор ключ-значение, где ключ - это название переменной/поля объекта, а значение - это структура, которая описывает переменную(или поле объекта). Причём, если переменная - это объект, тогда должно быть заполнено поле propeties(описание полей). Если переменная - массив, тогда должно быть заполнено поле items(описание типа, который хранится в массиве).
...
type FormParams struct {
	...
	// FormGroupId Form group id in SD
	FormGroupId *string `json:"form_group_id,omitempty"`

	// FormReEnterSettings Настройки блока при повторном заходе в этот блок
	FormReEnterSettings *FormReEnterSettings `json:"form_re_enter_settings,omitempty"`

	// FormsAccessibility List of accessibility properties for forms
	FormsAccessibility *[]FormsAccessibility `json:"forms_accessibility,omitempty"`

	// HideExecutorFromInitiator Hide executor from initiator
	HideExecutorFromInitiator bool `json:"hide_executor_from_initiator"`

	// IsEditable true - need manual feel when reenter in block
...
// JSONSchemaProperties Представляет из себя набор ключ-значение, где ключ - это название переменной/поля объекта, а значение - это структура, которая описывает переменную(или поле объекта). Причём, если переменная - это объект, тогда должно быть заполнено поле propeties(описание полей). Если переменная - массив, тогда должно быть заполнено поле items(описание типа, который хранится в массиве).
type JSONSchemaProperties map[string]struct {
	// Default Default value
	Default *JSONSchemaProperties_Default `json:"default,omitempty"`
	...
	// Required Required fields for type = object
	Required *[]string `json:"required,omitempty"`
...
// PlaceholderParams Placeholder block params
...
type ProcessSettings struct {
	...
	// ResubmissionPeriod Срок, в течении которого придет уведомление о том, что пользователь повторно создал заявку. Указывается в часах.
	ResubmissionPeriod int `json:"resubmission_period"`

	// Sla SLA в рабочих часах
	Sla         int         `json:"sla"`
	...
	// WorkType Рабочий режим
	WorkType ProcessSettingsWorkType `json:"work_type"`
}

// ProcessSettingsWorkType Рабочий режим
...
type ListPipelinesParams struct {
	...
	// IsPublished Show published pipelines only
	IsPublished *bool `form:"is_published,omitempty" json:"is_published,omitempty"`
	PerPage     *int  `form:"per_page,omitempty" json:"per_page,omitempty"`
	Page        *int  `form:"page,omitempty" json:"page,omitempty"`

	// Filter Фильтр по имени pipeline
	Filter *string `form:"filter,omitempty" json:"filter,omitempty"`
}
...
type PipelineNameExistsParams struct {
	...
	// CheckNotDeleted Check for not deleted pipelines
	CheckNotDeleted bool `form:"checkNotDeleted" json:"checkNotDeleted"`
}
...
type GetTasksParams struct {
	...
	// InitiatorLogins filter for initiators
	InitiatorLogins *[]string `form:"initiatorLogins,omitempty" json:"initiatorLogins,omitempty"`

	// ProcessingLogins filter in process by logins
	ProcessingLogins *[]string `form:"processingLogins,omitempty" json:"processingLogins,omitempty"`

	// ProcessingGroupIds filter in process by groups ids
	ProcessingGroupIds *[]string `form:"processingGroupIds,omitempty" json:"processingGroupIds,omitempty"`

	// ExecutorTypeAssigned filter type assigned
	ExecutorTypeAssigned *GetTasksParamsExecutorTypeAssigned `form:"executorTypeAssigned,omitempty" json:"executorTypeAssigned,omitempty"`
}
...
type SaveVersionMainSettingsJSONRequestBody = ProcessSettings
...
type SaveExternalSystemEndSettingsJSONRequestBody = EndSystemSettings
...
// AsPlaceholderParams returns the union data inside the Params as a PlaceholderParams
func (t Params) AsPlaceholderParams() (PlaceholderParams, error) {
	var body PlaceholderParams
	err := json.Unmarshal(t.union, &body)
	return body, err
}

// FromPlaceholderParams overwrites any union data inside the Params as the provided PlaceholderParams
func (t *Params) FromPlaceholderParams(v PlaceholderParams) error {
	b, err := json.Marshal(v)
	t.union = b
	return err
}

// MergePlaceholderParams performs a merge with any union data inside the Params, using the provided PlaceholderParams
func (t *Params) MergePlaceholderParams(v PlaceholderParams) error {
	b, err := json.Marshal(v)
	if err != nil {
		return err
	}
...
func (t Operand) ValueByDiscriminator() (interface{}, error) {
	...
	case "valueOperand":
		return t.AsValueOperand()
	case "variableOperand":
		return t.AsVariableOperand()
	default:
...
type ServerInterface interface {
	...
	DeleteExternalSystemEndSettings(w http.ResponseWriter, r *http.Request, versionID openapi_types.UUID, systemID openapi_types.UUID)
	// Save external systems settings for end of process
	// (PUT /pipelines/version/{versionID}/system/{systemID}/endRoutes)
	SaveExternalSystemEndSettings(w http.ResponseWriter, r *http.Request, versionID openapi_types.UUID, systemID openapi_types.UUID)
	// Delete Pipeline
