...
func (ae *APIEnv) GetTask(w http.ResponseWriter, req *http.Request, workNumber WorkNumber) {
	...
	dbTask, err := ae.DB.GetTask(ctx, string(workNumber))
	if err != nil {
		e := GetTaskError
...
func (ae *APIEnv) GetPipelineTasks(w http.ResponseWriter, req *http.Request, pipelineID PipelineID) {
	ctx, s := trace.StartSpan(req.Context(), "get_pipeline_tasks")
	defer s.End()

	log := logger.GetLogger(ctx)

	id, err := uuid.Parse(string(pipelineID))
	if err != nil {
		e := UUIDParsingError
		log.Error(e.errorMessage(err))
		_ = e.sendError(w)
...
func (ae *APIEnv) GetVersionTasks(w http.ResponseWriter, req *http.Request, versionID VersionID) {
...
func (ae *APIEnv) UpdateTask(w http.ResponseWriter, req *http.Request, workNumber WorkNumber) {
